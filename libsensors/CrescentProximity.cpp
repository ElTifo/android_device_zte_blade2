...
#include "CrescentProximity.h"

/*****************************************************************************/

CrescentProximity::CrescentProximity(char *dev)
...
CrescentProximity::~CrescentProximity() {
    if (mEnabled) {
        enable(ID_P,0);
    }
}

int CrescentProximity::initialise() {
...
int CrescentProximity::setInitialState() {
...
int CrescentProximity::enable(int32_t handle, int en) {
    if (handle != ID_P)
        return -EINVAL;
...
bool CrescentProximity::hasPendingEvents() const {
     return mPendingMask;
}

int CrescentProximity::readEvents(sensors_event_t* data, int count)
{
    if (count < 1)
        return -EINVAL;
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_ABS) {
            ...
            LOGE("CrescentSensor: unknown event (type=%d, code=%d)",type, event->code);
        }
        mInputReader.next();
    }
...
float CrescentProximity::indexToValue(size_t index) const
{
    return index;
}
