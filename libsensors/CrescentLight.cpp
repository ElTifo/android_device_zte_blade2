...
#include "CrescentLight.h"

/*****************************************************************************/

CrescentLight::CrescentLight(char *dev)
...
CrescentLight::~CrescentLight() {
    if (mEnabled) {
        enable(ID_L, 0);
    }
}

int CrescentLight::setInitialState() {
...
int CrescentLight::enable(int32_t handle, int en) {
...
bool CrescentLight::hasPendingEvents() const {
    return mPendingMask;
}

int CrescentLight::readEvents(sensors_event_t* data, int count)
{
    if (count < 1)
        return -EINVAL;
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_ABS) {
            ...
            LOGE("CrescentLight: unknown event (type=%d, code=%d)",type, event->code);
        }
        mInputReader.next();
    }
